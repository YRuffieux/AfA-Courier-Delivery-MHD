...
load(file=file.path(filepath_read,"tblBAS.RDAta"))       # baseline characteristics
load(file=file.path(filepath_read,"ICD10_F.RDAta"))      # mental health diagnoses
load(file=file.path(filepath_read,"tblFUPwide.RDAta"))   # start/end dates based on: database open/close date, enrolment dates, dates of entry to/exit from scheme
load(file=file.path(filepath_read,"tblREGIMEN.RData"))   # ART records between start and end dates
