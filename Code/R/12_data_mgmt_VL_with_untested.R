...
# "filling in" stretches of untsted follow-up
# takes 1-2 minutes
...
###### filling in stretches of follow-up with no testing, dummy VLs are inserted every six months
...
DTrna <- DTreg[DTrna[,.(patient,rna_d,rna_v,sex,birth_d,mhd_d,tested)],on=.(patient,art_sd<rna_d,art_ed>=rna_d)]
...
DTrna <- DTrna[,.(patient,sex,birth_d,rna_d=med_sd,mhd_d,rna_v,drug,art_type_cf,courier,courier_cat,
                  N_switches_arv=N_switches,art_first_line=as.numeric(art_line==1),tested)]
...
save(DTrna,file=file.path(filepath_processed,"AfA_VL_with_untested.RData"))
